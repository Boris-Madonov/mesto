export class Card {                                     // класс для создания карточек с фотографиями и подписями
    constructor(data, cardSelector, handlerCardClick) {  // передаем к класс объект data с необходимыми параметрами, селектор выбора карточки, обработчик нажатия по карточке
    ...
    _handlerLikeCard() {                                                                                 // приватный метод обработчика кнопки лайка карточки
        this._cardElement.querySelector('.element__like').classList.toggle('element__like_liked');
    }

    _handlerRemoveCard() {                                                                               // приватный метод обработчика кнопки удаления карточки
    ...
    _setEventListeners() {                                                                              // приватный метод слушателей карточки
        this._cardElement.querySelector('.element__like').addEventListener('click', () => {             // слушатель нажатия кнопки лайка карточки
            this._handlerLikeCard();                                                                     // вызов приватного метода обработчика кнопки лайка карточки
        });

        this._cardElement.querySelector('.element__remove').addEventListener('click', () => {           // слушатель нажатия на кнопку удаления карточки
            this._handlerRemoveCard();                                                                   // вызов приватного метода обработчика кнопки удаления карточки
        });
