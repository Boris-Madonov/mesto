export class Card {                                     // класс для создания карточек с фотографиями и подписями
    constructor(data, cardSelector, handleCardClick) {  // передаем к класс объект data с необходимыми параметрами, селектор выбора карточки, обработчик нажатия по карточке
